...
export class PlayerUI extends Component {
  ...
  public setRemainingHealth(amount: number, maxAmount: number, healthPercentage: number): void {
    this._timeSinceActivated = 0;
    const percentage = healthPercentage * 100;
    this._viewModel.healthAmountText = `${Math.max(0, Math.ceil(amount))}/${Math.ceil(maxAmount)}`;
