...
import { Stat, type PlayerStatsSaveData } from './PlayerStatsManager';
...
export class Game2 extends Component {
  ...
  private fetchData: Maybe<FetchData> = null;
  ...
  @subscribe(ReturnPVarEvent)
  private onReturnPVar(payload: ReturnPVarPayload): void {
    if (payload.player) this.playerDataEntity = payload.player;
    const stats = toSaveData(payload.stats);
    const playerStats = this.upgradeManager?.getPlayerStats();
    if (!playerStats) {
      this.pendingStats = stats;
      console.warn(`[Game2] ReturnPVar received, queued until playerStats ready`);
      return;
    }
    playerStats.applySaveData(stats);
    console.warn(`[Game2] applied loaded stats=${JSON.stringify(stats)}`);
  }
  ...
  private applyPendingStats(): void {
    ...
    playerStats.applySaveData(this.pendingStats);
    console.warn(`[Game2] applied queued stats=${JSON.stringify(this.pendingStats)}`);
  ...
  private applyUpgradeItem(upgradeItem?: UpgradeItem): void {
    ...
    const statNegative = upgradeItem.getStatNegative() as Stat;
    const valueNegative = upgradeItem.getValueNegative();
    const percentValueNegative = upgradeItem.getPercentValueNegative();

    if (valueNegative !== 0) playerStats.addStat(statNegative, -valueNegative);
    if (percentValueNegative !== 0) playerStats.addStatPercent(statNegative, -percentValueNegative);
  }
  ...
  public async startGame(): Promise<void> {
    ...
    if (this.player) {
      ...
      if (this.playerUI) {
        ...
        this.playerUI.setRemainingHealth(health.getHp(), health.getMax(), health.getHp() / health.getMax());
      }
    }
  ...
  private onPermanentUpgrade(data?: { stat: Stat; cost: number }): void {
    if (!data) return;
    console.warn(`[Game] Permanent upgrade: ${Stat[data.stat]} (cost: ${data.cost})`);
    const playerStats = this.upgradeManager?.getPlayerStats();
    if (!playerStats || !this.playerDataEntity) return;
    EventService.sendGlobally(
      SavePVarEvent,
      new SavePVarPayload(this.playerDataEntity, toStatsPayload(playerStats.serialize())),
    );
    console.warn(`[Game2] sent SavePVarEvent`);
  }
  ...
  private async onRetry(): Promise<void> {
    ...
    this.playerWeapons?.resetMultiShoot();
    this.updateXPUI();
  ...
  private setupUpgradeManager(): void {
    ...
    if (playerStats && this.player) {
      playerStats.unregisterDependent(this.player);
      playerStats.registerDependent(this.player);
    }
  ...
  private setupPlayerUI(): void {
    ...
    if (this.playerUI) {
      ...
      this.player.onHealed.on(this.onPlayerHealthChanged, this);
      this.player.onMaxChanged.on(this.onPlayerHealthChanged, this);
      console.log('[Game2] setupPlayerUI done');
    }
  }

  private onPlayerHealthChanged(): void {
  ...
  private onStartWave(waveIndex?: number): void {
    ...
    const gamePanel = this.uiManager.getPanel(GamePanel);
    gamePanel?.updateWaveString(this.waveManager.getDisplayWaveNumber(), this.waveManager.getTotalWaves());
  }
